import type {AnyGeneratedVideoData, NormalizedAssetData, TemplateId} from '@/types';
...
type TemplateFn = (asset: NormalizedAssetData, investment: number) => AnyGeneratedVideoData;
...
export function generateTemplateData(asset: NormalizedAssetData, template: TemplateId, investment: number) {
  if (template === 'COMPARE_ASSETS') {
    throw new Error('COMPARE_ASSETS requires two assets.');
  }

  const generator = templateMap[template];
  return generator(asset, investment);
}
...
export function generateComparisonData(
  ...
  return compareAssetsTemplate(primary, secondary, investment);
}
